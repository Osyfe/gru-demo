use gru_wgpu::ui::{self, Widget, widget::{primitive::{Label, Slider, Check}, layout::{Empty, Flex, FlexLayout}, WidgetExt}, lens};
use super::{Demo, light};

pub type UiEvent = ();

fn ui(data: &Demo) -> Box<dyn Widget<Demo, UiEvent>>
{
    if !data.ui
    {
        let open_text = Label::new().own("Open Lighting Menu");
        let open_check = Check::new().response().toggle().action(|request, _| request.widget()).lens(Demo::ui);
    ...
    let close_check = Check::new().response().toggle().action(|request, _| request.widget()).lens(Demo::ui);
...
pub fn build() -> ui::Ui<'static, Demo, UiEvent>
{
    ...
    ui::Ui::new(ui::UiInit::new(font), |_, data| ui(data))
}
